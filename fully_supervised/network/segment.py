...
class SegFormerModelWeakly(nn.Module):
    def __init__(self, n_classes=3):
        ...
        self.model.decode_head.classifier = nn.Conv2d(768, n_classes, kernel_size=(1, 1), stride=(1, 1))
        self.base_model = self.model.segformer
        # self.decode_head = torch.nn.Sequential(*(list(self.model.children())[-1:]))
        self.config = self.model.config
        # print(self.config)
        
        self.avgpool = nn.AdaptiveAvgPool2d(output_size=(1, 1))
        # self.cls = nn.Sequential(
        #     nn.Linear(512, 256, bias=True),  # 1024, 512 for resent18
        #     nn.ReLU(inplace=True),
        #     nn.Dropout(p=0.5, inplace=False),
            
        #     nn.Linear(256, 128, bias=True),  # 1024, 512 for resent18
        #     nn.ReLU(inplace=True),
        #     nn.Dropout(p=0.5, inplace=False),
