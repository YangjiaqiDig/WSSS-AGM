...
def train_once(args, epoch, trainloader, model, optimizer):
    ...
    for batch, data in enumerate(trainloader):
        ...
        loss_train, losses_group = get_mutil_task_loss(outputs, labels.to(args.device))
        loss_train.backward()
        optimizer.step()
    model.eval()
    ...
    train_acc_epoch, train_loss_epoch = {k: v  / (batch + 1) for k, v in total_acc.iteritems()}, total_loss / (batch + 1)
...
def valid_once(args, fold, epoch, testloader, model, optimizer):
    ...
    with torch.no_grad():
        for batch, data in enumerate(testloader):
            ...
            labels = data["labels"]
            outputs = model(image.to(args.device))
            loss_val, losses_group = get_mutil_task_loss(outputs, labels.to(args.device))
        ...
        # Print accuracy
        valid_acc_epoch, valid_loss_epoch = {k: v  / (batch + 1) for k, v in total_acc_val.iteritems()}, total_loss_val / (batch + 1)
