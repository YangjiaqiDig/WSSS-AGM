...
def tversky_loss(y_pred, y_true, alpha=0.7, beta=0.3, varepsilon=1.e-8):
    epsilon = 1.e-8
    y_true = y_true.float()
    y_pred = torch.clamp(y_pred, epsilon, 1. - epsilon)

    numerator = (y_true * y_pred).sum() + varepsilon
    denominator = y_true.sum() + alpha * (1 - y_true).sum() + beta * (1 - y_pred).sum() + varepsilon
...
class mIoULoss(nn.Module):
    ...
    def forward(self, inputs, target, cls_label):
        ...
        target_extend = target[:,0,:,:].unsqueeze(1).expand(N, self.n_foreground, target.shape[-2], target.shape[-1])
        target_bg = 1 - target[:,0]
        target_extend = torch.cat((target_extend, target_bg.unsqueeze(1)), dim=1)
        assert list(inputs.shape) == list(target_extend.shape), "target sizes must be b, 3, h, w."
        
        norm_inputs = max_norm(inputs)
        cls_mask = cls_label.unsqueeze(2).unsqueeze(3).expand(N, self.n_foreground+1, target.shape[-2], target.shape[-1])
        ...
        # Denominator
        union_bg = norm_inputs[:,-1] + target_extend[:,-1] - (norm_inputs[:,-1] * target_extend[:,-1])
        union_bg = union_bg.view(N, 1, -1).sum(2)
        loss_bg = inter[:,-1] / (union_bg + 1e-8)
        union_fg = norm_inputs[:,:-1] #+ target_extend[:,:-1] - (norm_inputs[:,:-1] * target_extend[:,:-1])
