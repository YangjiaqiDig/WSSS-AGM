...
from utils.utils import OrgLabels, convert_duke_labels, convert_our_dataset_labels, convert_resc_labels, get_num_lesions
...
def record_score(score, save_path):
    ...
    with open('iou_collections/{}_iou.txt'.format(save_path), 'w') as f:
        for num, cls_iou in enumerate(score_list):
            print('class {:2d} {:12} IU {:.2f}'.format(num, OrgLabels[num], round(cls_iou, 3)))
            f.write('class {:2d} {:12} IU {:.2f}'.format(num, OrgLabels[num], round(cls_iou, 3)) + '\n')
        print('meanIOU: ' + str(aveJ) + '\n')
        f.write('meanIOU: ' + str(aveJ) + '\n')    
        f.write('pixelAcc: ' + str(score['Pixel Accuracy']) + '\n' + 'meanAcc: ' + str(score['Mean Accuracy']) + '\n') 

'''mIoU'''
categories = ['background', 'SRF', 'IRF', 'EZ disrupted', 'HRD']# ['background', 'SRF', 'PED']   # ['background', 'Fluid'] #
