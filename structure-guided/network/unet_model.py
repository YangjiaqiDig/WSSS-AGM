...
class UNetWeakly(nn.Module):
    def __init__(self, n_classes, n_classes_layer=2, n_channels=3, bilinear=False):
        ...
        self.inc_layer = (DoubleConv(n_channels, 64))
    ...
    def cls_head_orig(self, feature):
        ...
        orig_cams = F.conv2d(feature, self.cls_orig.conv.weight)
    ...
    def forward(self, input_data):
        ...
        comb_cam = self.uncertainty_map(orig_cams, layer_cams, n_classes=self.n_classes)

        comb_f_out = [o + l for o, l in zip(orig_f_out, layer_f_out)]  # element-wise addition
        seg_res = self.decoder(comb_f_out)
        return orig_cls_res, layer_cls_res, seg_res,comb_cam
    
    def uncertainty_map(self, orig_CAM, layer_CAM, n_classes):
        ...
        layer_certainty = layer_prob[:, 1, :, :] # confidence of lesion, higher value lower certainty
        # Repeat the layer_certainty for n_lesions times along the channel dimension
        layer_certainty = layer_certainty.unsqueeze(1).repeat(1, n_classes-1, 1, 1)

        # Compute weighted CAMs
        weighted_CAMs = orig_CAM[:,1:] * layer_certainty # [b, n_classes-1, w, h], not include background
        # import pdb; pdb.set_trace()
        return weighted_CAMs
...
if __name__ == "__main__":
    ...
    orig_cls_res, layer_cls_res, seg_res, cams = model(input_data)
