...
class OCTDataset(Dataset):
    def __init__(self, args, data_type, infer_list=[], is_generate_pseudo_label=False):
        ...
        self.mask_list = {
            "train": glob.glob("{}/train/*.png".format(args.mask_dir)),
            "test": glob.glob("{}/test/*.png".format(args.mask_dir)),
        }
        if data_type == "inference":
            ...
            self.mask_list = {
                "inference": [
                    "{}/test/{}.png".format(args.mask_dir, item.split(".")[0])
                    for item in infer_list
                ]
            }
    ...
    def __getitem__(self, idx):
        ...
        image_tensor, mask_tensor = img_transform(
            image_arr, mask, self.args.is_size, transform_type
        )
...
class RESCDataset(Dataset):
    def __init__(self, args, data_type, infer_list=[], is_generate_pseudo_label=False):
        ...
        self.mask_list = {
            "train": glob.glob("{}/train/*".format(args.mask_dir)),
            "test": glob.glob("{}/valid/*".format(args.mask_dir)),
        }
        ...
        if data_type == "inference":
            ...
            self.mask_list = {
                "inference": [
                    "{}/valid/{}".format(args.mask_dir, item) for item in infer_list
                ]
            }
        self.data_type = data_type
        self.args = args
    ...
    def __getitem__(self, idx):
        ...
        image_label = (
            np.append(self.labels_list[img_name], [1])
            if "BackGround" in OrgLabels
            else self.labels_list[img_name]
        )
        return {
            "image": image_tensor,
...
class DukeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            "image": image_tensor,
            "labels": torch.FloatTensor([target_label, 1]),
            "path": target_path,
            "mask": mask_tensor,
            "shape": image_arr.shape,
            "mask_path": mask_path,
...
class SegmentRESCDataset(Dataset):
    def __init__(self, args, data_type, infer_list=[]):
        self.labels_list = {
            "train": glob.glob("{}/pseudo_label/*".format(args.root_dirs))
        }
        save_labels = np.load(
            f"{args.root_dirs}/resc_cls_labels.npy", allow_pickle=True
        ).item()
        test_label_paths = glob.glob("{}/valid/label_images/*".format(args.root_dirs))
        seg_label_paths = [
            x
            for x in test_label_paths
            if save_labels[x.split("/")[-1].split(".")[0]].sum() > 0
        ]
        self.labels_list["test"] = seg_label_paths
        if data_type == "inference":
            self.labels_list = {
                "inference": [
                    "{}/valid/label_images/{}".format(args.mask_dir, item)
                    for item in infer_list
                ]
            }
        self.data_type = data_type
    ...
    def __getitem__(self, idx):
        ...
        image_tensor, annot_tensor = img_transform(
            image, annot, self.args.is_size, self.data_type, is_seg=True
        )
        # import pdb; pdb.set_trace()
