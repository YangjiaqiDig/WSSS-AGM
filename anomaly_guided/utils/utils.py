...
class CAMGeneratorAndSave:
    ...
    def get_cam_results_per_class(self, orig_img, orig_mask, norm_annot):
        ...
        # print(self.model)
        # startt = time.time()
        # gb_model = GuidedBackpropReLUModel(model=self.model, use_cuda="cuda")
        # all_gb = []
        # for cls in range(len(OrgLabels)):
        #     if OrgLabels[cls] != "BackGround":
        #         gb = gb_model(extend_input_tensor, target_category=cls)
        #         gb = deprocess_image(gb)
        #         aa = cv2.cvtColor(gb[...,:3], cv2.COLOR_BGR2GRAY)
        #         all_gb.append(aa)
        # guided_cam = np.array(all_gb) / 255
        ...
        # grayscale_cam = guided_cam * grayscale_cam
            # print(grayscale_cam)
            # [cls, w, h]
        ...
        # map_save_pth = os.path.join(self.opts.save_folder, "cams")
        # np.save(f'{map_save_pth}/{n}.npy', save_file)
        # '''end'''
...
def get_seg_and_save(path, seg_out, labels, epoch, opts, save_results=False):
    ...
    for batch_nb, seg_pred in enumerate(seg_out):
        ...
        resize_seg = resize_seg[:-1] # remove background

        bg_score = [np.ones_like(orig_annot) * opts.out_cam_pred_alpha]
        pred_with_bg_score = np.concatenate((bg_score, resize_seg.detach().cpu().numpy()))
        # save_cam_map = {}
        # for _id, single_map in enumerate(pred_with_bg_score[1:]):
        #     if single_map.sum() != 0:
        #         save_cam_map[_id] = single_map
        # map_save_pth = os.path.join(opts.save_folder, "cams")
        # np.save(f'{map_save_pth}/{img_name.split(".")[0]}.npy', save_cam_map)

        binarized_seg = np.argmax(pred_with_bg_score, axis=0)  # [0 - num_class]
        save_seg = np.uint8(binarized_seg / get_num_classes() * 255)
        # import pdb; pdb.set_trace()
        ...
        gt.append(gt_labels)
        ready_pred_4d.append(binarized_seg)
        # import pdb; pdb.set_trace()
