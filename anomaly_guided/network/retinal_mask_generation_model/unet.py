...
class U_Net(nn.Module):
    def __init__(self):
        ...
        self.conv1_block = double_conv(3, 32)
    ...
    def forward(self, x):
        ...
        # print('befo÷re conv2', x.shape)
        # x = self.dropout(x)
        # Down 2
        conv2 = self.conv2_block(x)
        # print('after ÷conv2', conv2.shape)
        x = self.maxpool(conv2)
        # print('bef÷ore conv3', x.shape)

        # Down 3
        conv3 = self.conv3_block(x)
        # print('after c÷onv3', conv3.shape)
        ...
        x = self.conv_up_2(x)
        # print('after conv_3', x.s÷hape)

        # Up 3
        x = self.up_3(x)
        # print('up_3', x.÷shape)
        x = torch.cat([x, conv2], dim=1)
        # print('after cat_÷2', x.shape)
        x = self.conv_up_3(x)
        ...
        # print('after ca÷t_1', x.shape)
        x = self.conv_up_4(x)
        # print('after co÷nv_1', x.shape)

        # Final output
        x = self.conv_final(x)
        # print('final:÷ ', x.shape)
        likelihood_map = self.softmax(x)#[:,1,:,:]
        # print(likeliho÷od_map.shape)
